...
public class VendingMachineTest {
	...
	private static Vending VM = new Vending();
	...
	// insertion of coins
	public static void inputCoins(Scanner sc, Vending VM){
		System.out.println("Insert coins in the form of: NICKLE,PENNY, DIME and QUARTER");
		String coins = sc.nextLine();
		Balance = (double) VM.drop(coins,Balance);
	}

	//selection of items
	public static String selectItems(Scanner sc, Vending VM){
	...
	public static void initiateInventory(){
		for(Item i : Item.values()){
			itemInventory.put(i.getName(), 4);
		}
		for(Coin i : Coin.values()){
			coinInventory.put(i.getName(), 4);
		}
	}
